...
var slider = null; // Declare slider here to make it accessible in the global scope
var buttonBar;
var newButton = null;



['urlchange', 'load', 'DOMContentLoaded'].forEach(function (e) {
  window.addEventListener(e, function () {
    if (window.location.href.startsWith("https://www.linkedin.com/in/")) {
      ...
      async function addButton() {
        ...
        buttonBar = document.querySelector('.pv-top-card-v2-ctas > div');

        if (slider == null) {

          if (buttonBar) {
            newButton = document.getElementById("showProfile");
            if (newButton == null) {
              ...
              Promise.all([
              ...
              ]).then(async ([fullName, about, experiences, headline, url, imgurl, contactInfo]) => {
                ...
                await addProfile(fullName, about, experiences, headline, url, contactInfo.phone, imgurl, contactInfo.email)
    ...
    // Close the slider when the url is not a LinkedIn profile page
    else if (!window.location.href.startsWith("https://www.linkedin.com/in/")) {
      try {
        slider.style.transform = 'translateX(100%)';
        document.body.removeChild(slider);
        buttonBar.removeChild(newButton);
        slider = null;
        newButton = null;
      } catch (e) {
        console.log(e);
...
async function addProfile(fullName, about, experiences, headline, url, mobile, imgurl, email) {
...
function extractProfileImageLink() {
  // Select the img element inside the button with class .pv-top-card-profile-picture
  let imgElement = document.querySelector('.pv-top-card-profile-picture img');

  // If the img element exists, return its src attribute, else return an empty string
